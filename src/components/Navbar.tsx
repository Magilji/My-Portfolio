  ...
  return (
    <motion.header 
      ...
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        isScrolled ? 'bg-portfolio-bg/90 backdrop-blur shadow-md py-4' : 'py-6'
      }`}
    >
      ...
      <motion.div 
        ...
        className="md:hidden fixed top-0 right-0 h-screen w-3/4 bg-portfolio-secondary shadow-lg pt-20 px-6"
      >
          ...
          <ol className="flex flex-col space-y-6">
            {navItems.map((item, i) => (
              <li key={i}>
                <a 
                  href={item.href}
                  className="nav-link text-lg font-mono block"
